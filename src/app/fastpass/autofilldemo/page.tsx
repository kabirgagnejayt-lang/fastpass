...
export default function FastPassAutofillDemoPage() {
    const [finalStatus, setFinalStatus] = useState<'approved' | 'declined' | null>(null);

    const clientApp = {
        name: "Demo Autofill App",
        description: "wants to use FastPass to pre-fill your info.",
        verified: true,
        logo: ""
    }
    ...
    const requestedData = [
        ...
        { key: "email", label: "Email Address", value: userProfile.email },
    ]

    const sendPopupMessage = (status: "approved" | "declined" | "canceled") => {
        const data = status === 'approved' ? { name: userProfile.name, email: userProfile.email } : null;
        ...
        <div className="flex min-h-screen items-center justify-center bg-background p-4">
            <Card className="w-full max-w-md">
                <CardHeader className="text-center">
                    ...
                    <CardDescription>{clientApp.description}</CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                    <div className="rounded-lg border p-4 space-y-4">
                        <p className="text-sm font-medium text-muted-foreground">This app will receive the following information:</p>
                        <div className="space-y-2">
                            {requestedData.map(({ key, label, value }) => (
                    ...
                    <Alert variant="destructive">
                        ...
                        <AlertDescription>
                            No actual data will be stored or shared. This is a simulated experience.
                        </AlertDescription>
                    </Alert>
                </CardContent>
            </Card>
