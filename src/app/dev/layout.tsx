...
export default function DevLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    { href: "/dev", icon: AppWindow, label: "My Apps" },
    { href: "/dev/create", icon: PlusCircle, label: "Create App" },
    { href: "/dev/docs", icon: Book, label: "Documentation" },
  ];
  
  if (isAdmin) {
    navItems.push({ href: "/dev/manage", icon: ShieldCheck, label: "App Management" });
  }
  
  const isManagePage = /^\/dev\/[^/]+$/.test(pathname) && !navItems.some(item => item.href === pathname);
  let pageTitle = navItems.find(item => pathname.startsWith(item.href))?.label || 'Developer Panel';
  if (isManagePage) {
    pageTitle = "Edit App";
  }
          ...
          {user && (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuItem onClick={() => router.push('/dev/settings')}>Settings</DropdownMenuItem>
                <DropdownMenuSeparator />
