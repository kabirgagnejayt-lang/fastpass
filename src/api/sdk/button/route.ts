...
export async function GET(request: NextRequest) {
  ...
  const buttonCss = `
    ...
    .fastpass-button-wrapper { position: relative; width: 100%; max-width: 350px; min-height: 90px; }
  ...
  const script = `
    (function() {
      // Check if a special hook exists for the test page. If so, let it handle setup.
      if (typeof (window as any).onFastPassScriptLoad === 'function') {
        (window as any).onFastPassScriptLoad({
            clientId: "${clientId}",
            buttonCss: \`${buttonCss}\`,
        ...
        return; // Stop execution to let test page take over
      }
      ...
      function openPopup() {
        ...
        url.searchParams.append('openerOrigin', openerOrigin);
        
        popup = window.open(url.toString(), "fastpass-popup", \`width=\${width},height=\${height},top=\${top},left=\${left}\`);
      ...
      window.addEventListener("message", (event) => {
          ...
          if (event.origin !== "${hostUrl}") {
              // Allow messages from the current origin for testing/development
